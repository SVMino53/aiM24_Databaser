...
SELECT
    d.DepartmentName,
    SUM(s.BaseSalary) TotalSalaries
FROM
    Salaries s
...
SELECT
	...
	s.BaseSalary,
	s.Bonus,
	s.BaseSalary + s.Bonus AS TotalCompensationPerMonth,
    SUM(s.BaseSalary + s.Bonus) OVER (PARTITION BY e.EmployeeID) AS TotalCompensation
FROM
    Salaries s
JOIN Employees e ON e.EmployeeID = s.EmployeeID;


-- Wijdans variant som löste ett... liknande problem
...
SELECT
	...
	s.BaseSalary,
	s.Bonus,
    SUM(s.BaseSalary + s.Bonus) OVER (PARTITION BY s.SalaryDate, e.FirstName) AS TotalCompensationPerMonth
FROM
    Salaries s
JOIN Employees e ON e.EmployeeID = s.EmployeeID;
...
FROM
	Employees e JOIN Salaries s ON e.EmployeeID = s.EmployeeID;